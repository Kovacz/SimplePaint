...
using ScrollOffset = struct {
    double x;
    double y;
};
using CursorPosition = struct {
    double x;
    double y;
};
using KeyboardAction = struct {
    int key;
    int scancode;
    int mods;
};
...
class Event {
    using EventData = std::variant<ScrollOffset, CursorPosition, KeyboardAction>;
...
public:
    ...
    EventType getEventType() const noexcept { return m_type; }
