...
Texture::Texture() : m_data(nullptr), m_width(0), m_height(0), m_nrChannels(0), m_handle(0) {
...
Texture::info Texture::getInfo() const noexcept { return std::make_pair(m_width, m_height); }
...
unsigned Texture::getHandle() const noexcept { return m_handle; }

void Texture::bind(const Texture* texture) { glBindTexture(GL_TEXTURE_2D, texture ? texture->getHandle() : 0); }
